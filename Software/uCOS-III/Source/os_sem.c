                   ...
                   OS_ERR      *p_err)
{
    ...
    /* Not allowed to be called from an ISR        中断服务子程序不能调用OSSemCreate函数，所有建立信号量的工作必须在任务级代码中，或者多任务启动前完成         */
...
#if (OS_CFG_CALLED_FROM_ISR_CHK_EN == DEF_ENABLED)
    if (OSIntNestingCtr > 0u) {
    	/* Not allowed to call from an ISR       检查是否是中断服务程序调用的， 中断服务程序不能等待，              */
#if (defined(TRACE_CFG_EN) && (TRACE_CFG_EN == DEF_ENABLED))
...
//如果信号量cnt<1
    ...
    //将任务pending                     /* Lock the scheduler/re-enable interrupts              */
    OS_CRITICAL_ENTER_CPU_EXIT();
    ...
    //如果信号量有效或者超时，在调用OSSched之后会继续执行下面的部分
    CPU_CRITICAL_ENTER();
